...
-(id)init {
    ...
    if (self) {
        ...
        printf("Setting variable segment_penalty_dict_nonword failed!!!\n");
        if (!tesseract->SetVariable("segment_penalty_garbage","10"))
        printf("Setting variable segment_penalty_garbage failed!!!\n");
        if (!tesseract->SetVariable("stopper_nondict_certainty_base","-100"))
        printf("Setting variable stopper_nondict_certainty_base failed!!!\n");
        if (!tesseract->SetVariable("language_model_penalty_non_dict_word","1"))
        printf("Setting variable language_model_penalty_non_dict_word failed!!!\n");
        if (!tesseract->SetVariable("language_model_penalty_non_freq_dict_word","1"))
        printf("Setting variable language_model_penalty_non_freq_dict_word failed!!!\n");
        if (!tesseract->SetVariable("GARBAGE_STRING","5"))
        printf("Setting variable GARBAGE_STRING failed!!!\n");
        if (!tesseract->SetVariable("NON_WERD","5"))
        printf("Setting variable NON_WERD failed!!!\n");
...
/*
    NSLog(@"BEFORE ORIENTATION %d", uiImage.imageOrientation);
    
    if(uiImage.imageOrientation == UIImageOrientationRight) {
        uiImage = [uiImage imageRotatedByDegrees:-90.0];
    }
    else if(uiImage.imageOrientation == UIImageOrientationLeft) {
        uiImage = [uiImage imageRotatedByDegrees:90.0];
    }
    else if (uiImage.imageOrientation == UIImageOrientationDown) {
        uiImage = [uiImage imageRotatedByDegrees:180.0];
    }
    
    NSLog(@"AFTER ORIENTATION %d", uiImage.imageOrientation);
*/
